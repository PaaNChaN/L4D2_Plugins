...
public void OnPluginStart()
{
    ...
    RegConsoleCmd("sm_unp",     Unpause_Cmd, "Marks your team as ready for an unpause");
    RegConsoleCmd("sm_up",      Unpause_Cmd, "Marks your team as ready for an unpause");
    ...
    RegConsoleCmd("sm_unrdy",   Unready_Cmd, "Marks your team as ready for an unpause");
    RegConsoleCmd("sm_urdy",    Unready_Cmd, "Marks your team as ready for an unpause");
    RegConsoleCmd("sm_unr",     Unready_Cmd, "Marks your team as ready for an unpause");
    RegConsoleCmd("sm_ur",      Unready_Cmd, "Marks your team as ready for an unpause");
    RegConsoleCmd("sm_u",       Unready_Cmd, "Marks your team as ready for an unpause");
    RegConsoleCmd("sm_toggleready", ToggleReady_Cmd, "Toggles your team's ready status");
    RegConsoleCmd("sm_tr",          ToggleReady_Cmd, "Toggles your team's ready status");
    ...
    RegAdminCmd("sm_forceup",      ForceUnpause_Cmd, ADMFLAG_BAN, "Unpauses the game regardless of team ready status.  Must be used to unpause admin pauses");
    RegAdminCmd("sm_fup",          ForceUnpause_Cmd, ADMFLAG_BAN, "Unpauses the game regardless of team ready status.  Must be used to unpause admin pauses");
...
public void OnClientPutInServer(int client)
{
    if (isPaused)
    {
        ...
        {
            CPrintToChatAll("{default}<SM> {olive}%N {default}has fully loaded", client);
        }
    }
}
...
public OnMapStart()
{
        ConVar l4d_ready_live_sound = FindConVar("l4d_ready_live_sound");
...
public Action Pause_Cmd(int client, int args)
{
    ...
    {
        CPrintToChatAll("{default}<SM> {olive}%N {blue}Paused{default}.", client);
...
public Action PauseDelay_Timer(Handle timer)
{
    if (pauseDelay == 0)
    {
        CPrintToChatAll("{default}<SM> {red}PAUSED");
        AttemptPause();
        return Plugin_Stop;
    }
    else
    {
        CPrintToChatAll("{default}<SM> {blue}Pausing in{default}: {olive}%d", pauseDelay);
        pauseDelay--;
    }
    ...
    if (isPaused && IsPlayer(client))
    {
        ...
        if (!teamReady[clientTeam])
        {
            switch (clientTeam)
            {
                case L4D2Team_Survivor:
                    CPrintToChatAll("{default}<SM> {olive}%N %s{default}marked {blue}%s {default}ready.", client, (g_bEnableInitReady && client == initiator) ? "{default}as {green}Initiator " : "", teamString[clientTeam]);
                case L4D2Team_Infected:
                    CPrintToChatAll("{default}<SM> {olive}%N %s{default}marked {red}%s {default}ready.", client, (g_bEnableInitReady && client == initiator) ? "{default}as {green}Initiator " : "", teamString[clientTeam]);                   
            }
        }
        if (g_bEnableInitReady)
        {
            if (client == initiator && !g_bInitiatorReady)
            {
                ...
                if (teamReady[clientTeam])
                {
                    CPrintToChatAll("{default}<SM> {olive}%N {default}marked {green}Initiator {default}ready.", client);
                }
            }
        }
        ...
        if (CheckFullReady())
        {
            if (!adminPause) {
                ...
                CPrintToChatAll("{default}<SM> {olive}Teams {default}are ready. Wait for {blue}Admin {default}to {green}confirm{default}.");
            }
        }
    }
}
...
public Action Unready_Cmd(int client, int args)
{
    if (isPaused && IsPlayer(client))
    {
        ...
        if (teamReady[clientTeam])
        {
            ...
            {
                case L4D2Team_Survivor:
                    CPrintToChatAll("{default}<SM> {olive}%N %s{default}marked {blue}%s {default}not ready.", client, (g_bEnableInitReady && client == initiator) ? "{default}as {green}Initiator " : "", teamString[clientTeam]);
                case L4D2Team_Infected:
                    CPrintToChatAll("{default}<SM> {olive}%N %s{default}marked {red}%s {default}not ready.", client, (g_bEnableInitReady && client == initiator) ? "{default}as {green}Initiator " : "", teamString[clientTeam]);
            }
        }
        if (g_bEnableInitReady)
        {
            if (client == initiator && g_bInitiatorReady)
            {
                ...
                if (!teamReady[clientTeam])
                {
                    CPrintToChatAll("{default}<SM> {olive}%N {default}marked {green}Initiator {default}not ready.", client);
                }
            }
        }
...
{
    if (isPaused && hiddenPanel[client])
    {
        hiddenPanel[client] = false;
        hiddenManually[client] = false;
...
AttemptPause()
{
    if (deferredPauseTimer == null)
    {
        ...
        {
            CPrintToChatAll("{default}<SM> {red}Pause has been delayed due to a pick-up in progress!");
        ...
        if (IsClientInGame(client) && !IsFakeClient(client))
        {
            if (L4D2_Team:GetClientTeam(client) == L4D2Team_Infected && IsGhost(client))
            {
                ...
                if (buttons & IN_ATTACK)
                {
                    ...
                    CPrintToChat(client, "{default}<SM> {default}Your {red}Spawn {default}has been prevented because of the Pause");
                }
            }
            if (!pauseProcessed)
            {
...
UpdatePanel()
{
    ...
    if (sn_main_name != null)
        sn_main_name.GetString(sHostName, sizeof(sHostName));
    else FindConVar("hostname").GetString(sHostName, sizeof(sHostName));

    char sHostNum[10];
    ConVar sn_host_num = FindConVar("sn_host_num");
    if (sn_host_num != null)
        sn_host_num.GetString(sHostNum, sizeof(sHostNum));

    char sCfgName[30];
    FindConVar("l4d_ready_cfg_name").GetString(sCfgName, sizeof(sCfgName));

    Format(info, sizeof(info), "%s %s %d/%d [ %s ]", sHostName, sHostNum, GetSeriousClientCount(), FindConVar("sv_maxplayers").IntValue, sCfgName);
    ...
    if (numb < 60)
        Format(info, sizeof(info), "Pause Duration: 0m %is", numb);
    else
    {
        int min = numb/60;
        int sec = numb-min*60;
        
        Format(info, sizeof(info), "Pause Duration: %im %is", min, sec);
    }
    menuPanel.DrawText(info);
    ...
    {
        menuPanel.DrawText(g_bInitiatorReady ? "->1. Initiator: [✔]" : "->1. Initiator: [✗]");
        menuPanel.DrawText(teamReady[L4D2Team_Survivor] ? "->2. Survivors: [✔]" : "->2. Survivors: [✗]");
        menuPanel.DrawText(teamReady[L4D2Team_Infected] ? "->3. Infected: [✔]" : "->3. Infected: [✗]");
    } 
    else
    ...
    menuPanel.DrawText("▸ !hide/!show -> Hide/Show this HUD");
...
void InitiateLiveCountdown()
{
    ...
    {
        CPrintToChatAll("{default}<SM> Say {olive}!unready {default}to cancel");
...
public Action ReadyCountdownDelay_Timer(Handle timer)
{
    ...
    {
        Unpause();
        EmitSoundToAll(liveSound, _, SNDCHAN_AUTO, SNDLEVEL_NORMAL, SND_NOFLAGS, 1.0);
        PrintHintTextToAll("Round (%d/2) Continue!", InSecondHalfOfRound() + 1);
        numb = 0;
        return Plugin_Stop;
    }
    else
    {
        CPrintToChatAll("{default}<SM> {blue}Live in{default}: {olive}%d{default}...", readyDelay);
        readyDelay--;
    }
    return Plugin_Continue;
}
...
CancelFullReady(client)
{
    if (readyCountdownTimer != null && !adminPause)
    {
        ...
        readyCountdownTimer = null;
        CPrintToChatAll("{default}<SM> {olive}%N {default}cancelled the countdown!", client);
    }
}
...
public Action Callvote_Callback(int client, char[] command, int argc)
{
    if (L4D2_Team:GetClientTeam(client) == L4D2Team_Spectator)
    {
        CPrintToChat(client, "<SM> {default}You're unable to call votes as a spectator.");
        return Plugin_Handled;
    }
    if (IgnorePlayer[client] > 0)
    {
        CPrintToChat(client, "<SM> {default}You've just switched Teams, you are unable to vote for a few seconds.");
        return Plugin_Handled;
    }
    ...
    CPrintToChat(client, "<SM> {default}You may not kick Admins.", target);
